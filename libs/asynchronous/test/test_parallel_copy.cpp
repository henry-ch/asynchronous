...
struct Servant : boost::asynchronous::trackable_servant<>
{
    ...
    boost::shared_future<void> start_async_work()
    {
        ...
        post_callback(
           [ids,this](){
                    ...
                    return boost::asynchronous::parallel_copy(this->m_data.begin(), this->m_data.end(), this->m_target.begin(), 1500);
                    },// work
    ...
    boost::shared_future<void> start_async_work_range_moved()
    {
        ...
        post_callback(
           [ids,this](){
                    ...
                    BOOST_CHECK_MESSAGE(contains_id(ids.begin(),ids.end(),boost::this_thread::get_id()),"task executed in the wrong thread");
                    return boost::asynchronous::parallel_copy(std::move(this->m_data), this->m_target.begin(), 1500);
                    },// work
    ...
    boost::shared_future<void> start_async_work_continuations()
    {
        BOOST_CHECK_MESSAGE(main_thread_id!=boost::this_thread::get_id(),"servant start_async_work_range not posted.");
        // we need a promise to inform caller when we're done
        boost::shared_ptr<boost::promise<void> > aPromise(new boost::promise<void>);
...
BOOST_AUTO_TEST_CASE( test_parallel_copy )
{
    servant_dtor=false;
    {
...
BOOST_AUTO_TEST_CASE( test_parallel_copy_range_moved )
{
    servant_dtor=false;
    {
        auto scheduler = boost::asynchronous::make_shared_scheduler_proxy<boost::asynchronous::single_thread_scheduler<
        ...
        main_thread_id = boost::this_thread::get_id();
        ServantProxy proxy(scheduler);
        boost::shared_future<boost::shared_future<void> > fuv = proxy.start_async_work_range_moved();
        try
        {
            boost::shared_future<void> resfuv = fuv.get();
            resfuv.get();
...
BOOST_AUTO_TEST_CASE( test_parallel_copy_continuations )
{
    servant_dtor=false;
    {
        auto scheduler = boost::asynchronous::make_shared_scheduler_proxy<boost::asynchronous::single_thread_scheduler<
        ...
        main_thread_id = boost::this_thread::get_id();
        ServantProxy proxy(scheduler);
        boost::shared_future<boost::shared_future<void> > fuv = proxy.start_async_work_continuations();
        try
        {
            boost::shared_future<void> resfuv = fuv.get();
            resfuv.get();
