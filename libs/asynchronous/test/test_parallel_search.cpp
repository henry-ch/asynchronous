...
struct Servant : boost::asynchronous::trackable_servant<>
{
    ...
    boost::future<void> start_async_work_ok()
    {
        ...
        post_callback(
           [ids,this](){
                    ...
                    return boost::asynchronous::parallel_search(this->m_data.begin(),this->m_data.end(),
           ...
           [aPromise,ids,this](boost::asynchronous::expected<Iterator> res){
                        ...
                        auto it = std::search(this->m_data.begin(),this->m_data.end(),
                                              this->m_data2.begin(),this->m_data2.end(),
                                              [](int i, int j){return i == j;});

                        BOOST_CHECK_MESSAGE(it != m_data.end(),"parallel_search should have found value.");
                        BOOST_CHECK_MESSAGE(it == res.get(),"parallel_search found wrong value.");
                        aPromise->set_value();
           }// callback functor.
        );
        return fu;
    }
    boost::future<void> start_async_work_nok()
    {
        ...
        post_callback(
           ...
           [aPromise,ids,this](boost::asynchronous::expected<Iterator> res){
                        ...
                        BOOST_CHECK_MESSAGE(it == m_data.end(),"parallel_search should not have found value.");
                        BOOST_CHECK_MESSAGE(it == res.get(),"parallel_search found wrong value.");
                        aPromise->set_value();
           }// callback functor.
        );
    ...
    boost::future<void> start_async_work_continuation()
    {
        BOOST_CHECK_MESSAGE(main_thread_id!=boost::this_thread::get_id(),"servant start_async_work_ok not posted.");
        m_data2.clear();
    ...
    boost::future<void> start_async_work_ok_no_fct()
    {
        BOOST_CHECK_MESSAGE(main_thread_id!=boost::this_thread::get_id(),"servant start_async_work_ok not posted.");
        m_data2.clear();
        m_data2.push_back(m_data[3100]);
    ...
    boost::future<void> start_async_work_ok2()
    {
        BOOST_CHECK_MESSAGE(main_thread_id!=boost::this_thread::get_id(),"servant start_async_work_ok not posted.");
        m_data2.clear();
        m_data2.push_back(m_data[2999]);
...
BOOST_AUTO_TEST_CASE( test_parallel_search_ok )
{
    servant_dtor=false;
    {
        ...
        boost::future<boost::future<void> > fuv = proxy.start_async_work_ok();
        try
        {
...
BOOST_AUTO_TEST_CASE( test_parallel_search_nok )
{
    servant_dtor=false;
    {
        auto scheduler = boost::asynchronous::make_shared_scheduler_proxy<boost::asynchronous::single_thread_scheduler<
        ...
        main_thread_id = boost::this_thread::get_id();
        ServantProxy proxy(scheduler);
        boost::future<boost::future<void> > fuv = proxy.start_async_work_nok();
        try
        {
            boost::future<void> resfuv = fuv.get();
            resfuv.get();
...
BOOST_AUTO_TEST_CASE( test_parallel_search_ok2 )
{
    servant_dtor=false;
    {
        auto scheduler = boost::asynchronous::make_shared_scheduler_proxy<boost::asynchronous::single_thread_scheduler<
        ...
        main_thread_id = boost::this_thread::get_id();
        ServantProxy proxy(scheduler);
        boost::future<boost::future<void> > fuv = proxy.start_async_work_ok2();
        try
        {
            boost::future<void> resfuv = fuv.get();
            resfuv.get();
...
BOOST_AUTO_TEST_CASE( test_parallel_search_ok_no_fct )
{
    servant_dtor=false;
    {
        auto scheduler = boost::asynchronous::make_shared_scheduler_proxy<boost::asynchronous::single_thread_scheduler<
        ...
        main_thread_id = boost::this_thread::get_id();
        ServantProxy proxy(scheduler);
        boost::future<boost::future<void> > fuv = proxy.start_async_work_ok_no_fct();
        try
        {
            boost::future<void> resfuv = fuv.get();
            resfuv.get();
...
BOOST_AUTO_TEST_CASE( test_parallel_search_continuation )
{
    servant_dtor=false;
    {
        auto scheduler = boost::asynchronous::make_shared_scheduler_proxy<boost::asynchronous::single_thread_scheduler<
        ...
        main_thread_id = boost::this_thread::get_id();
        ServantProxy proxy(scheduler);
        boost::future<boost::future<void> > fuv = proxy.start_async_work_continuation();
        try
        {
            boost::future<void> resfuv = fuv.get();
            resfuv.get();
