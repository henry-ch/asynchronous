...
const std::string caesar = "Gallia est omnis divisa in partes tres, quarum unam incolunt Belgae, aliam Aquitani, tertiam qui ipsorum lingua Celtae, nostra Galli appellantur. Hi omnes lingua, institutis, "
...
const std::string caesar_search_string = "Gallia";
...
#define AS_RNGE(arg) const_cast<const decltype(arg)&>(arg) /* Make this unmoveable */
#define AS_MOVE(arg) std::move(typename std::decay<decltype(arg)>::type { AS_RNGE(arg) } /* Make a copy and move that */)
...
#define TEST_RUN(haystack, needle, modif_a, modif_b) \
    do { \
        /* Copy haystack and needle for AS_MOVE runs */ \
        auto haystack_copy = haystack; \
        /* Compute reference solution in O(n^2) time */ \
        auto reference = string_search(haystack_copy, needle); \
        /* Run KMP and wait for results. */ \
        KMP_IMPL(AS_##modif_a(haystack), AS_##modif_b(needle)); \
        /* Verify that we found the same results. */ \
