...
struct Servant : boost::asynchronous::trackable_servant<>
{
    ...
    boost::shared_future<void> do_sort(std::vector<SORTED_TYPE> a, size_t n)
    {
        ...
        post_callback(
               ...
               [this](boost::asynchronous::expected<std::vector<SORTED_TYPE>> /*res*/){
                            this->on_callback();
               }// callback functor.
               ,"",0,0
        );
    ...
    boost::shared_future<void> do_spreadsort(std::vector<SORTED_TYPE> a, size_t n)
    {
        boost::shared_future<void> fu = m_promise->get_future();
        long tasksize = NELEM / tasks;
        servant_time = boost::chrono::high_resolution_clock::now();
...
void ParallelAsyncPostCb(std::vector<SORTED_TYPE> a, size_t n)
{
                                                                             ...
                                                                             boost::asynchronous::default_save_cpu_load<10,80000,1000>>(tpsize));
    {
        ...
        boost::shared_future<boost::shared_future<void> > fu = proxy.do_sort(std::move(a),n);
...
void ParallelAsyncPostCbSpreadsort(std::vector<SORTED_TYPE> a, size_t n)
{
                                                                             ...
                                                                             boost::asynchronous::default_save_cpu_load<10,80000,1000>>(tpsize));
    {
        ...
        boost::shared_future<boost::shared_future<void> > fu = proxy.do_spreadsort(std::move(a),n);
        boost::shared_future<void> resfu = fu.get();
        resfu.get();
    }
}
