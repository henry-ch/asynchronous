    ...
    boost::shared_future<void> start_parallel_stable_sort_moved_range()
    {
        ...
        post_callback(
           [tp,this](){
                    ...
                    return boost::asynchronous::parallel_stable_sort_move(std::move(this->m_data),increasing_sort_subtask(),1500);
                    },// work
    ...
    boost::shared_future<void> start_parallel_sort_moved_range()
    {
        ...
        post_callback(
           [tp,this](){
                    ...
                    std::vector<boost::thread::id> ids = tp.thread_ids();
                    BOOST_CHECK_MESSAGE(contains_id(ids.begin(),ids.end(),boost::this_thread::get_id()),"task executed in the wrong thread");
                    return boost::asynchronous::parallel_sort_move(std::move(this->m_data),increasing_sort_subtask(),1500);
                    },// work
           [aPromise,tp,data_copy,this](boost::asynchronous::expected<std::vector<int>> res) mutable{
                        BOOST_CHECK_MESSAGE(!res.has_exception(),"servant work threw an exception.");
                        BOOST_CHECK_MESSAGE(main_thread_id!=boost::this_thread::get_id(),"servant callback in main thread.");
...
BOOST_AUTO_TEST_CASE( test_parallel_reverse_sorted_2 )
{
    servant_dtor=false;
    {
...
BOOST_AUTO_TEST_CASE( test_parallel_already_sorted_2 )
{
    servant_dtor=false;
    {
        auto scheduler = boost::asynchronous::create_shared_scheduler_proxy(new boost::asynchronous::single_thread_scheduler<
...
BOOST_AUTO_TEST_CASE( test_parallel_stable_sort_range_2 )
{
    servant_dtor=false;
    {
        auto scheduler = boost::asynchronous::create_shared_scheduler_proxy(new boost::asynchronous::single_thread_scheduler<
...
BOOST_AUTO_TEST_CASE( test_parallel_stable_sort_moved_range_2 )
{
    servant_dtor=false;
    {
        auto scheduler = boost::asynchronous::create_shared_scheduler_proxy(new boost::asynchronous::single_thread_scheduler<
...
BOOST_AUTO_TEST_CASE( test_parallel_sort_post_future_2 )
{
    ...
    boost::future<std::vector<int>> fu = boost::asynchronous::post_future(
                scheduler,
                [data]() mutable {return boost::asynchronous::parallel_sort_move(std::move(data),increasing_sort_subtask(),1500);});
    try
    {
