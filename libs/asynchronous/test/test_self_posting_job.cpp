...
struct DummyJob
{
    DummyJob(std::shared_ptr<std::promise<void> > done,std::string const& name=""):m_done(done),m_name(name){}
    void operator()()const
    {
    ...
    std::shared_ptr<std::promise<void> > m_done;
...
BOOST_AUTO_TEST_CASE( self_posting_job_threadpool_scheduler)
{
    ...
    std::future<void> fu = done->get_future();
