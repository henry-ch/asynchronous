...
struct Servant : boost::asynchronous::trackable_servant<>
{
    ...
    std::future<void> test_inclusive_scan()
    {
        ...
        post_callback(
           [ids,this](){
                    ...
                    return boost::asynchronous::parallel_inclusive_scan(m_data1.begin(),m_data1.end(),m_data2.begin(),0,
                                                                        std::plus<int>(),
                                                                        100);
                    },// work
    ...
    std::future<void> test_inclusive_scan_moved_ranges()
    {
        BOOST_CHECK_MESSAGE(main_thread_id!=boost::this_thread::get_id(),"servant async work not posted.");
        generate(m_data1,1000,700);
        m_data2 = std::vector<int>(1000,0);
        ...
        post_callback(
           [ids,this](){
                    BOOST_CHECK_MESSAGE(main_thread_id!=boost::this_thread::get_id(),"servant work not posted.");
                    BOOST_CHECK_MESSAGE(contains_id(ids.begin(),ids.end(),boost::this_thread::get_id()),"task executed in the wrong thread");
                    return boost::asynchronous::parallel_inclusive_scan(std::move(m_data1),std::move(m_data2),0,
                                                                        std::plus<int>(),
                                                                        100);
                    },// work
    ...
    std::future<void> test_inclusive_scan_moved_range()
    {
        BOOST_CHECK_MESSAGE(main_thread_id!=boost::this_thread::get_id(),"servant async work not posted.");
        generate(m_data1,1000,700);
        m_data2 = std::vector<int>(1000,0);
        ...
        post_callback(
           [ids,this](){
                    BOOST_CHECK_MESSAGE(main_thread_id!=boost::this_thread::get_id(),"servant work not posted.");
                    BOOST_CHECK_MESSAGE(contains_id(ids.begin(),ids.end(),boost::this_thread::get_id()),"task executed in the wrong thread");
                    return boost::asynchronous::parallel_inclusive_scan(std::move(m_data1),0,
                                                                        std::plus<int>(),
                                                                        100);
                    },// work
    ...
    std::future<void> test_inclusive_scan_continuation()
    {
        BOOST_CHECK_MESSAGE(main_thread_id!=boost::this_thread::get_id(),"servant async work not posted.");
        generate(m_data1,1000,700);
        m_data2 = std::vector<int>(1000,0);
        ...
        post_callback(
           [ids,this](){
                    BOOST_CHECK_MESSAGE(main_thread_id!=boost::this_thread::get_id(),"servant work not posted.");
                    BOOST_CHECK_MESSAGE(contains_id(ids.begin(),ids.end(),boost::this_thread::get_id()),"task executed in the wrong thread");
                    return boost::asynchronous::parallel_inclusive_scan(boost::asynchronous::parallel_for(std::move(m_data1),
                                                                                                          [](int& i)
                                                                                                          {
                                                                                                            i += 2;
                                                                                                          },100),
...
public:
    ...
    BOOST_ASYNC_FUTURE_MEMBER(test_inclusive_scan_moved_ranges)
    BOOST_ASYNC_FUTURE_MEMBER(test_inclusive_scan_moved_range)
    BOOST_ASYNC_FUTURE_MEMBER(test_inclusive_scan_continuation)
};
}

BOOST_AUTO_TEST_CASE( test_inclusive_scan )
{
    servant_dtor=false;
    {
        ...
        auto fuv = proxy.test_inclusive_scan();
        try
        {
            auto resfuv = fuv.get();
            resfuv.get();
...
BOOST_AUTO_TEST_CASE( test_inclusive_scan_moved_ranges )
{
    servant_dtor=false;
    {
        auto scheduler = boost::asynchronous::create_shared_scheduler_proxy(new boost::asynchronous::single_thread_scheduler<
        ...
        main_thread_id = boost::this_thread::get_id();
        ServantProxy proxy(scheduler);
        auto fuv = proxy.test_inclusive_scan_moved_ranges();
        try
        {
            auto resfuv = fuv.get();
            resfuv.get();
...
BOOST_AUTO_TEST_CASE( test_inclusive_scan_moved_range )
{
    servant_dtor=false;
    {
        auto scheduler = boost::asynchronous::create_shared_scheduler_proxy(new boost::asynchronous::single_thread_scheduler<
        ...
        main_thread_id = boost::this_thread::get_id();
        ServantProxy proxy(scheduler);
        auto fuv = proxy.test_inclusive_scan_moved_range();
        try
        {
            auto resfuv = fuv.get();
            resfuv.get();
...
BOOST_AUTO_TEST_CASE( test_inclusive_scan_continuation )
{
    servant_dtor=false;
    {
        auto scheduler = boost::asynchronous::create_shared_scheduler_proxy(new boost::asynchronous::single_thread_scheduler<
        ...
        main_thread_id = boost::this_thread::get_id();
        ServantProxy proxy(scheduler);
        auto fuv = proxy.test_inclusive_scan_continuation();
        try
        {
            auto resfuv = fuv.get();
            resfuv.get();
