...
struct Servant : boost::asynchronous::trackable_servant<>
{
    ...
    boost::future<void> test_exclusive_scan()
    {
        ...
        post_callback(
           [ids,this](){
                    ...
                    return boost::asynchronous::parallel_exclusive_scan(m_data1.begin(),m_data1.end(),m_data2.begin(),0,
                                                                        std::plus<int>(),
                                                                        100);
                    },// work
    ...
    boost::future<void> test_exclusive_scan_moved_ranges()
    {
        BOOST_CHECK_MESSAGE(main_thread_id!=boost::this_thread::get_id(),"servant async work not posted.");
        generate(m_data1,1000,700);
        m_data2 = std::vector<int>(1000,0);
        ...
        post_callback(
           [ids,this](){
                    BOOST_CHECK_MESSAGE(main_thread_id!=boost::this_thread::get_id(),"servant work not posted.");
                    BOOST_CHECK_MESSAGE(contains_id(ids.begin(),ids.end(),boost::this_thread::get_id()),"task executed in the wrong thread");
                    return boost::asynchronous::parallel_exclusive_scan(std::move(m_data1),std::move(m_data2),0,
                                                                        std::plus<int>(),
                                                                        100);
                    },// work
...
public:
    ...
    BOOST_ASYNC_FUTURE_MEMBER(test_exclusive_scan_moved_ranges)
};
}

BOOST_AUTO_TEST_CASE( test_exclusive_scan )
{
    servant_dtor=false;
    {
        ...
        boost::future<boost::future<void> > fuv = proxy.test_exclusive_scan();
        try
        {
...
BOOST_AUTO_TEST_CASE( test_exclusive_scan_moved_ranges )
{
    servant_dtor=false;
    {
        auto scheduler = boost::asynchronous::create_shared_scheduler_proxy(new boost::asynchronous::single_thread_scheduler<
        ...
        main_thread_id = boost::this_thread::get_id();
        ServantProxy proxy(scheduler);
        boost::future<boost::future<void> > fuv = proxy.test_exclusive_scan_moved_ranges();
        try
        {
            boost::future<void> resfuv = fuv.get();
            resfuv.get();
