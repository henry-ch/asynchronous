...
struct Servant : boost::asynchronous::trackable_servant<>
{
    ...
    boost::shared_future<void> parallel_copy_if()
    {
        generate(m_data);
        ...
        post_callback(
           [ids,this](){
                    ...
                    return boost::asynchronous::parallel_copy_if(
           ...
           [aPromise,ids,this](boost::asynchronous::expected<Iterator> res)mutable{
                        ...
                        std::vector<int>::iterator it = std::copy_if(this->m_data.begin(),this->m_data.end(),dest.begin(),
                                               [](int i){return i < 2000;});

                        auto itres = res.get();
                        BOOST_CHECK_MESSAGE((std::distance(dest.begin(),it) == std::distance(m_dest.begin(),itres)),
                                            "parallel_copy_if is wrong");

                        m_dest.erase(itres,m_dest.end());
                        dest.erase(it,dest.end());
                        BOOST_CHECK_MESSAGE(dest == m_dest,"parallel_copy_if copy went wrong");
...
public:
    ...
    {}
    BOOST_ASYNC_FUTURE_MEMBER(parallel_copy_if)
};

}

BOOST_AUTO_TEST_CASE( test_parallel_copy_if )
{
    servant_dtor=false;
    {
        ...
        boost::shared_future<boost::shared_future<void> > fuv = proxy.parallel_copy_if();
        try
        {
