...
using channel_topic_t = boost::asynchronous::subscription::channel_topic<>;
...
struct Servant : boost::asynchronous::trackable_servant<>
{
    ...
    void force_unsubscribe_string_topic(std::string const& topic = "")
    {
    ...
    void force_unsubscribe_channel_topic(std::string const& topic = "")
    {
        unsubscribe<some_event>(token_, channel_topic_t{ topic });
        unsubscribe<some_event>(token2_, channel_topic_t{ topic });
        // check cleanup
        BOOST_CHECK_MESSAGE(boost::asynchronous::subscription::get_waiting_subscribes().empty(), "missing cleanup in waiting subscribes/topic");
    }
...
public:
    ...
    BOOST_ASYNC_FUTURE_MEMBER(force_unsubscribe_string_topic)
    BOOST_ASYNC_FUTURE_MEMBER(force_unsubscribe_channel_topic)
    BOOST_ASYNC_FUTURE_MEMBER(cb_called)
    BOOST_ASYNC_FUTURE_MEMBER(wait_for_some_event_exact_topic)
    BOOST_ASYNC_FUTURE_MEMBER(wait_for_some_event_channel_topic)
};
...
struct Servant2 : boost::asynchronous::trackable_servant<>
{
    ...
    void trigger_some_event_channel_topic(std::string const& topic)
    {
        this->publish(some_event{ 42 }, channel_topic_t{ topic });
    }
...
public:
    ...
    BOOST_ASYNC_FUTURE_MEMBER(trigger_some_event_channel_topic)
    BOOST_ASYNC_FUTURE_MEMBER(wait_for_some_event)
};
...
BOOST_AUTO_TEST_CASE(test_full_notification_exact_topic_match)
{
    ...
    try
    {
        ...
        proxy->force_unsubscribe_string_topic("topic1").get();
        BOOST_CHECK_MESSAGE(proxy->cb_called().get() == 1, "got wrong number of events");
    }
    catch (...)
    {
...
BOOST_AUTO_TEST_CASE(test_full_notification_exact_channel_topic_match)
{
    auto scheduler1 = boost::asynchronous::make_shared_scheduler_proxy<boost::asynchronous::single_thread_scheduler<
        boost::asynchronous::guarded_deque<>>>();
    auto scheduler2 = boost::asynchronous::make_shared_scheduler_proxy<boost::asynchronous::single_thread_scheduler<
    ...
    try
    {
        ...
        auto res_fu = proxy->wait_for_some_event_channel_topic("A/B").get();
        wait_for_subscribe(proxy2);
...
BOOST_AUTO_TEST_CASE(test_full_notification_channel_topic_no_match)
{
    auto scheduler1 = boost::asynchronous::make_shared_scheduler_proxy<boost::asynchronous::single_thread_scheduler<
        boost::asynchronous::guarded_deque<>>>();
    auto scheduler2 = boost::asynchronous::make_shared_scheduler_proxy<boost::asynchronous::single_thread_scheduler<
    ...
    try
    {
        ...
        auto res_fu = proxy->wait_for_some_event_channel_topic("A/B").get();
        wait_for_subscribe(proxy2);
        proxy2.trigger_some_event_channel_topic("A/C").get();
