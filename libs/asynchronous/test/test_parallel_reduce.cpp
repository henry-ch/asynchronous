...
struct Servant : boost::asynchronous::trackable_servant<>
{
    ...
    std::future<void> start_async_work_range_fct()
    {
        BOOST_CHECK_MESSAGE(main_thread_id!=boost::this_thread::get_id(),"servant start_async_work not posted.");
        // we need a promise to inform caller when we're done
        std::shared_ptr<std::promise<void> > aPromise(new std::promise<void>);
    ...
    std::future<void> start_async_work_range_range_fct()
    {
        BOOST_CHECK_MESSAGE(main_thread_id!=boost::this_thread::get_id(),"servant start_async_work_range not posted.");
        // we need a promise to inform caller when we're done
        std::shared_ptr<std::promise<void> > aPromise(new std::promise<void>);
    ...
    std::future<void> start_async_work_range2_range_fct()
    {
        BOOST_CHECK_MESSAGE(main_thread_id!=boost::this_thread::get_id(),"servant start_async_work_range2 not posted.");
        // we need a promise to inform caller when we're done
        std::shared_ptr<std::promise<void> > aPromise(new std::promise<void>);
    ...
    std::future<void> start_async_work_range3_range_fct()
    {
        BOOST_CHECK_MESSAGE(main_thread_id!=boost::this_thread::get_id(),"servant start_async_work_range3 not posted.");
        // we need a promise to inform caller when we're done
        std::shared_ptr<std::promise<void> > aPromise(new std::promise<void>);
    ...
    std::future<void> test_empty_range()
    {
        BOOST_CHECK_MESSAGE(main_thread_id!=boost::this_thread::get_id(),"servant start_async_work not posted.");
        // we need a promise to inform caller when we're done
        std::shared_ptr<std::promise<void> > aPromise(new std::promise<void>);
...
BOOST_AUTO_TEST_CASE( test_parallel_reduce_empty_range )
{
    servant_dtor=false;
    {
        ...
        main_thread_id = boost::this_thread::get_id();
        ServantProxy proxy(scheduler);
        auto fuv = proxy.test_empty_range();
        try
        {
            auto resfuv = fuv.get();
            resfuv.get();
...
BOOST_AUTO_TEST_CASE( test_parallel_reduce_range_fct )
{
    servant_dtor=false;
    {
        ...
        main_thread_id = boost::this_thread::get_id();
        ServantProxy proxy(scheduler);
        auto fuv = proxy.start_async_work_range_fct();
        try
        {
            auto resfuv = fuv.get();
            resfuv.get();
...
BOOST_AUTO_TEST_CASE( test_parallel_reduce_range_range_fct )
{
    servant_dtor=false;
    {
        ...
        main_thread_id = boost::this_thread::get_id();
        ServantProxy proxy(scheduler);
        auto fuv = proxy.start_async_work_range_range_fct();
        try
        {
            auto resfuv = fuv.get();
            resfuv.get();
...
BOOST_AUTO_TEST_CASE( test_parallel_reduce_range2_range_fct )
{
    servant_dtor=false;
    {
        ...
        main_thread_id = boost::this_thread::get_id();
        ServantProxy proxy(scheduler);
        auto fuv = proxy.start_async_work_range2_range_fct();
        try
        {
            auto resfuv = fuv.get();
            resfuv.get();
...
BOOST_AUTO_TEST_CASE( test_parallel_reduce_range3_range_fct )
{
    servant_dtor=false;
    {
        ...
        main_thread_id = boost::this_thread::get_id();
        ServantProxy proxy(scheduler);
        auto fuv = proxy.start_async_work_range3_range_fct();
        try
        {
            auto resfuv = fuv.get();
            resfuv.get();
