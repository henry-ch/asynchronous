...
struct Servant : boost::asynchronous::trackable_servant<>
{
    ...
    boost::shared_future<void> start_async_work()
    {
        ...
        post_callback(
           ...
           [aPromise,ids,this](boost::asynchronous::expected<long> res){
    ...
    boost::shared_future<void> start_async_work2()
    {
        BOOST_CHECK_MESSAGE(main_thread_id!=boost::this_thread::get_id(),"servant start_async_work not posted.");
        // we need a promise to inform caller when we're done
        boost::shared_ptr<boost::promise<void> > aPromise(new boost::promise<void>);
        ...
        post_callback(
           [ids,this](){
                    ...
                    BOOST_CHECK_MESSAGE(contains_id(ids.begin(),ids.end(),boost::this_thread::get_id()),"task executed in the wrong thread");
                    return boost::asynchronous::parallel_count(this->m_data.begin(),this->m_data.end(),20,20);
                    },// work
           [aPromise,ids,this](boost::asynchronous::expected<long> res){
                        BOOST_CHECK_MESSAGE(!res.has_exception(),"servant work threw an exception.");
                        BOOST_CHECK_MESSAGE(main_thread_id!=boost::this_thread::get_id(),"servant callback in main thread.");
                        ...
                        BOOST_CHECK_MESSAGE(!res.has_exception(),"servant work threw an exception.");
                        long r = res.get();
                        BOOST_CHECK_MESSAGE((r == 1), ("result of parallel_count was " + std::to_string(r) + ", should have been 20"));
                        // reset
                        m_data = mkdata();
                        aPromise->set_value();
           }// callback functor.
    ...
    boost::shared_future<void> start_async_work_range()
    {
        ...
        post_callback(
           [ids,this](){
                    ...
                    BOOST_CHECK_MESSAGE(contains_id(ids.begin(),ids.end(),boost::this_thread::get_id()),"task executed in the wrong thread");
                    return boost::asynchronous::parallel_count_if(const_cast<std::vector<int>const&>(this->m_data),
                                                           [](int const& a)
                                                           {
                                                             return a < 40 && a >= 20;
                                                           },20);
    ...
    boost::shared_future<void> start_async_work_range2()
    {
        ...
        post_callback(
           [ids,this](){
                    ...
                    BOOST_CHECK_MESSAGE(contains_id(ids.begin(),ids.end(),boost::this_thread::get_id()),"task executed in the wrong thread");
                    return boost::asynchronous::parallel_count_if(std::move(this->m_data),
                                                           [](int const& a)
                                                           {
                                                             return a < 40 && a >= 20;
                                                           },20);
    ...
    boost::shared_future<void> start_async_work_range2b()
    {
        BOOST_CHECK_MESSAGE(main_thread_id!=boost::this_thread::get_id(),"servant start_async_work_range2b not posted.");
        // we need a promise to inform caller when we're done
        boost::shared_ptr<boost::promise<void> > aPromise(new boost::promise<void>);
        boost::shared_future<void> fu = aPromise->get_future();
        boost::asynchronous::any_shared_scheduler_proxy<> tp =get_worker();
        ...
        post_callback(
           [ids,this](){
                    ...
                    BOOST_CHECK_MESSAGE(contains_id(ids.begin(),ids.end(),boost::this_thread::get_id()),"task executed in the wrong thread");
                    return boost::asynchronous::parallel_count(std::move(this->m_data),20,20);
                    },// work
           [aPromise,ids,this](boost::asynchronous::expected<long> res){
                        BOOST_CHECK_MESSAGE(!res.has_exception(),"servant work threw an exception.");
                        BOOST_CHECK_MESSAGE(main_thread_id!=boost::this_thread::get_id(),"servant callback in main thread.");
        ...
        post_callback(
           [ids,this](){
                    ...
                    return boost::asynchronous::parallel_count_if(
                                boost::asynchronous::parallel_find_all(std::move(this->m_data),
                                                                       [](int const& a)
                                                                       {
                                                                          return a < 40 && a >= 20;
                                                                       }, 20),
                                [](int const& a) { return (a % 2 == 0); }, 20);
                    },// work
           [aPromise,ids,this](boost::asynchronous::expected<long> res){
                        BOOST_CHECK_MESSAGE(!res.has_exception(),"servant work threw an exception.");
                        BOOST_CHECK_MESSAGE(main_thread_id!=boost::this_thread::get_id(),"servant callback in main thread.");
    ...
    boost::shared_future<void> start_async_work_range3b()
    {
        BOOST_CHECK_MESSAGE(main_thread_id!=boost::this_thread::get_id(),"servant start_async_work_range3 not posted.");
        // we need a promise to inform caller when we're done
        boost::shared_ptr<boost::promise<void> > aPromise(new boost::promise<void>);
        ...
        post_callback(
           ...
           [aPromise,ids,this](boost::asynchronous::expected<long> res){
                        ...
                        BOOST_CHECK_MESSAGE(!res.has_exception(),"servant work threw an exception.");
                        long r = res.get();
                        BOOST_CHECK_MESSAGE((r == 1), ("result of parallel_count was " + std::to_string(r) + ", should have been 1"));
                        // reset
                        m_data = mkdata();
                        aPromise->set_value();
           }// callback functor.
...
public:
    ...
    BOOST_ASYNC_FUTURE_MEMBER(start_async_work_range2b)
    BOOST_ASYNC_FUTURE_MEMBER(start_async_work_range3)
    BOOST_ASYNC_FUTURE_MEMBER(start_async_work_range3b)
};
...
BOOST_AUTO_TEST_CASE( test_parallel_count2 )
{
    servant_dtor=false;
    {
        auto scheduler = boost::asynchronous::make_shared_scheduler_proxy<boost::asynchronous::single_thread_scheduler<
        ...
        main_thread_id = boost::this_thread::get_id();
        ServantProxy proxy(scheduler);
        boost::shared_future<boost::shared_future<void> > fuv = proxy.start_async_work2();
        try
        {
            boost::shared_future<void> resfuv = fuv.get();
            resfuv.get();
...
BOOST_AUTO_TEST_CASE( test_parallel_count_range2b )
{
    servant_dtor=false;
    {
        auto scheduler = boost::asynchronous::make_shared_scheduler_proxy<boost::asynchronous::single_thread_scheduler<
        ...
        main_thread_id = boost::this_thread::get_id();
        ServantProxy proxy(scheduler);
        boost::shared_future<boost::shared_future<void> > fuv = proxy.start_async_work_range2b();
        try
        {
            boost::shared_future<void> resfuv = fuv.get();
            resfuv.get();
...
BOOST_AUTO_TEST_CASE( test_parallel_count_range3b )
{
    servant_dtor=false;
    {
        auto scheduler = boost::asynchronous::make_shared_scheduler_proxy<boost::asynchronous::single_thread_scheduler<
        ...
        main_thread_id = boost::this_thread::get_id();
        ServantProxy proxy(scheduler);
        boost::shared_future<boost::shared_future<void> > fuv = proxy.start_async_work_range3b();
        try
        {
            boost::shared_future<void> resfuv = fuv.get();
            resfuv.get();
