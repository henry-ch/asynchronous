...
struct Servant : boost::asynchronous::trackable_servant<>
{
    ...
    boost::future<void> start_parallel_all_of_true()
    {
        ...
        post_callback(
           [ids,this](){
                    ...
                    return boost::asynchronous::parallel_all_of(this->m_data.begin(),this->m_data.end(),
                                                                [](int i){return i < 1001;},1500);
                    },// work
           [aPromise,ids,data_copy,this](boost::asynchronous::expected<bool> res) mutable{
                        ...
                        BOOST_CHECK_MESSAGE(!res.has_exception(),"servant work threw an exception.");
                        bool res2 = std::all_of(data_copy.begin(),data_copy.end(),[](int i){return i < 1001;});
    ...
    boost::future<void> start_parallel_all_of_false()
    {
        BOOST_CHECK_MESSAGE(main_thread_id!=boost::this_thread::get_id(),"start_parallel_all_of_false not posted.");
        // we need a promise to inform caller when we're done
        std::shared_ptr<boost::promise<void> > aPromise(new boost::promise<void>);
        boost::future<void> fu = aPromise->get_future();
        boost::asynchronous::any_shared_scheduler_proxy<> tp =get_worker();
        ...
        post_callback(
           ...
           [aPromise,ids,data_copy,this](boost::asynchronous::expected<bool> res) mutable{
                        ...
                        BOOST_CHECK_MESSAGE(!contains_id(ids.begin(),ids.end(),boost::this_thread::get_id()),"task callback executed in the wrong thread(pool)");
                        BOOST_CHECK_MESSAGE(!res.has_exception(),"servant work threw an exception.");
                        bool res2 = std::all_of(data_copy.begin(),data_copy.end(),[](int i){return i < 10;});
                        BOOST_CHECK_MESSAGE(res2 == res.get(),"parallel_all_of gave a wrong value.");
                        // reset
                        generate(this->m_data);
                        aPromise->set_value();
    ...
    boost::future<void> start_parallel_all_of_continuation()
    {
        BOOST_CHECK_MESSAGE(main_thread_id!=boost::this_thread::get_id(),"start_parallel_all_of_true not posted.");
        // we need a promise to inform caller when we're done
        std::shared_ptr<boost::promise<void> > aPromise(new boost::promise<void>);
...
#else
    BOOST_ASYNC_FUTURE_MEMBER_1(start_parallel_all_of_true)
    BOOST_ASYNC_FUTURE_MEMBER_1(start_parallel_all_of_false)
    BOOST_ASYNC_FUTURE_MEMBER_1(start_parallel_all_of_continuation)
#endif
};

}

BOOST_AUTO_TEST_CASE( test_parallel_all_of_true )
{
    servant_dtor=false;
    {
        ...
        boost::future<boost::future<void> > fuv = proxy.start_parallel_all_of_true();
        try
        {
...
BOOST_AUTO_TEST_CASE( test_parallel_all_of_false )
{
    servant_dtor=false;
    {
        auto scheduler = boost::asynchronous::make_shared_scheduler_proxy<boost::asynchronous::single_thread_scheduler<
        ...
        main_thread_id = boost::this_thread::get_id();
        ServantProxy proxy(scheduler);
        boost::future<boost::future<void> > fuv = proxy.start_parallel_all_of_false();
        try
        {
            boost::future<void> resfuv = fuv.get();
            resfuv.get();
...
BOOST_AUTO_TEST_CASE( test_parallel_all_of_continuation )
{
    servant_dtor=false;
    {
        ...
        main_thread_id = boost::this_thread::get_id();
        ServantProxy proxy(scheduler);
        boost::future<boost::future<void> > fuv = proxy.start_parallel_all_of_continuation();
        try
        {
            boost::future<void> resfuv = fuv.get();
            resfuv.get();
