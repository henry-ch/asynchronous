...
#ifndef ASIO_HTTP_ASYNC_CLIENT_HPP
#define ASIO_HTTP_ASYNC_CLIENT_HPP
  ...
  void handle_read_headers(const boost::system::error_code& err)
  {
    if (!err)
    {
      ...
      while (std::getline(response_stream, header) && header != "\r")
        response_data_ << header << "\n";
      response_data_ << "\n";

      // Write whatever content we already have to output.
      if (response_.size() > 0)
        response_data_ << &response_;
...
#endif /* ASIO_HTTP_ASYNC_CLIENT_HPP */
