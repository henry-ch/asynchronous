...
#include "dummy_parallel_find_all_task.hpp"
using namespace std;
...
struct Servant : boost::asynchronous::trackable_servant<boost::asynchronous::any_callable,boost::asynchronous::any_serializable>
{
    ...
    void on_callback(std::vector<int> const& result)
    {
        ...
        std::cout << "m_data[" << result.size() - 1 << "]= " << *it << std::endl;
        // inform test caller
        m_promise->set_value();
    }
    ...
    boost::shared_future<void> start_async_work()
    {
        ...
        post_callback(
               dummy_parallel_find_all_task(),// work
...
void example_find_all_tcp()
{
    std::cout << "example_find_all_tcp" << std::endl;
    {
    ...
    std::cout << "end example_find_all_tcp \n" << std::endl;
}
