...
public:
    ...
    typedef stealing_multiqueue_threadpool_scheduler<Q,FindPosition,IsImmediate> this_type;
    ...
    void init(size_t number_of_workers,std::vector<boost::asynchronous::any_queue_ptr<job_type> > const& others,boost::weak_ptr<this_type> weak_self)
    {
        ...
        for (size_t i = 0; i< number_of_workers;++i)
        {
            ...
            boost::thread* new_thread =
                    m_group->create_thread(boost::bind(&stealing_multiqueue_threadpool_scheduler::run,this->m_queues,m_private_queue,others,i,m_diagnostics,fu,weak_self));
    ...
    static void run(std::vector<boost::shared_ptr<queue_type> > queues,boost::shared_ptr<boost::asynchronous::threadsafe_list<job_type> > private_queue,
